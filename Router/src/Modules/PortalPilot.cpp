...
namespace Modules {
	//----------
	PortalPilot::PortalPilot(shared_ptr<ModuleControl> moduleControl)
	...
	//----------
	void
		PortalPilot::populateInspector(ofxCvGui::InspectArguments& args)
	{
	...
	{
		{
			...
			this->moduleControl->move(ModuleControl::Axis::A, steps);
		}

		{
			...
			auto steps = (int32_t)ofMap(norm
				...
				, 0
				, -this->parameters.axes.microstepsPerPrismRotation.get());
			this->moduleControl->move(ModuleControl::Axis::B, steps);
		}
