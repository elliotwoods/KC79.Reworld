...
driving two prism-rotation motor axes). It covers the physical bus, framing,
serialisation, reliability model, the Ethernet/RS485-gateway transport, the
separate ID-assignment bus, and the firmware update protocol.
...
3. [RS485 over Ethernet (the TCP gateway)](#3-rs485-over-ethernet-the-tcp-gateway)
4. [Packetisation: COBS framing](#4-packetisation-cobs-framing)
5. [Serialisation: the MessagePack envelope](#5-serialisation-the-messagepack-envelope)
6. [Reliability: ACKs, timeouts, collation](#6-reliability-acks-timeouts-collation)
7. [The ID daisy-chain](#7-the-id-daisy-chain)
8. [Firmware update / bootloader protocol](#8-firmware-update--bootloader-protocol)
...
one RS485 bus (or its Ethernet-gateway equivalent — see §3) and a number of
...
**framed** with **COBS** (Consistent Overhead Byte Stuffing, see §4) so a
...
- **Framing:** 8 data bits, no parity, 1 stop bit ("**8N1**") — the default
  ...
  detected at the UART level at all (see §9).
...
gateway transport (§3) toggles a direction pin. This means the USB↔RS485
...
| ID chain TX | PB8 | USART3 | separate bus, see §7 |
| ID chain RX | PB9 | USART3 | separate bus, see §7 |
| ID DIP switches | PD0–PD3 | GPIO, `INPUT_PULLUP` | fallback address, see §7 |

(Full pin map: `PortalFW/pins.md`.)

---

## 3. RS485 over Ethernet (the TCP gateway)
...
upstream by `RS485::serialThreadSend` exactly as for direct serial (§4).
...
etc. (§6) are unaffected by which transport a Column uses; the gateway is
invisible to everything above the `IDevice` abstraction.

---

## 4. Packetisation: COBS framing
...
integer 0, or a null/`nil` value), we need a way to remove all zero bytes
from a message before sending it, in a way that a receiver can perfectly
reverse.
...
worked example, straight from the vendored codec's own test vectors
(`Router/src/cobs-c/`, ported byte-for-byte in
...
Wrapped in the 3-element envelope `[-1, 0, body]` (see §5 for exactly how
target/source are encoded):
...
This 10-byte plaintext contains exactly one zero byte (the source, `00`, at
index 2). Encoding it with COBS:
...
(Portal 1), body `{"p": [94848, 0, 94848, 0]}`.

---

## 5. Serialisation: the MessagePack envelope
...
- `source` (int): who sent it (`0` from the Router; the sending Portal's
  own ID otherwise).
- `body`: the payload — see the shape catalog below.
...
**Worked example — unicast move command.** Body `{"m": [94848, 0]}` (move
Portal 8's axis A to position 94848, axis B to 0), addressed unicast:
...
COBS-framed, this becomes `03 93 08 06 81 A1 6D 92 CE 04 01 72 80 01 00` —
note two zero bytes in the plaintext (the fixint `00` for axis B, and the
high byte of `94848` = `0x00_01_72_80`) both get transformed away and
restored transparently on decode.

### Message body shapes

| Body | MessagePack shape | Meaning |
|---|---|---|
| `nil` | `C0` | **Ping** — elicits a bare ACK, nothing else. |
| `string5`, e.g. `"FW"` | `A2 46 57` | A **magic word** — see §8 (firmware update / reboot-to-bootloader). |
| bare `bool` | `C2`/`C3` | **ACK** — success/failure of the previous command. Note this is not wrapped in a map, and is indistinguishable in shape from any other bare-value body (see §9). |
| `{"p": [a, b, ta, tb]}` | 1-entry fixmap, value = 4×int32 fixarray | **Position reply** — current position and target position for both axes. |
| `{"app":…, "mca":…, "mcb":…, "logger":…}` | up to 4-entry fixmap | **Full status reply** to a `poll` — app uptime/version/calibration, per-axis motion-control health, recent log messages. |
| `{"poll": nil}` | 1-entry fixmap | Request a full status reply. |
| `{"p": nil}` | 1-entry fixmap | Request just a position reply (cheaper, higher-frequency poll). |
| `{"m": [a, b]}` | 1-entry fixmap, array of 1–2 int32 | Move both axes (or just one, if only one element given). |
| `{"motionControlA": {…}}` / `"motionControlB"` | nested fixmap | Per-axis motion commands: `move`, `motionProfile`, `zeroCurrentPosition`, `measureBacklash`, `home`, `initTimer`, `deinitTimer`, `testTimer`. |
| `{"motorDriverA": {…}}` / `"motorDriverB"` | nested fixmap | `testRoutine`, `testTimer`. |
| `{"motorDriverSettings": {…}}` | nested fixmap | `setCurrent` (float32 amps), `setMicrostepResolution` (log2 of the resolution, e.g. 32 → 5). |
| `{"init": …}` / `{"calibrate": …}` / `{"home": …}` / `{"unjam": …}` | value = `nil` (defaults) or array `[timeout_s, slowSpeed, backOffDistance, debounceDistance(, tryCount)]` | Long-running calibration routines — see the early-ACK note in §6. |
| `{"flashLED": …}` | value = `nil` or `[period_us, count]` | Blink the status LED, for physically identifying a board. |
| `{"debugLightsEnabled": bool}` | | Toggle debug LEDs. |
| `{"escapeFromRoutine": nil}` | | Abort whatever long routine is currently running. |
| `{"reset": nil}` | | Reboot the application (not the bootloader — a normal `NVIC_SystemReset()`; contrast with the `"FW"` magic word in §8). |
...
(`App::processIncomingByKey`, `PortalFW/src/Modules/App.cpp`). The `"id"`
key is currently parsed but not acted on by anything (`ID` doesn't
implement a handler for it) — a stub for future use, not a bug in current
behaviour.
...
## 6. Reliability: ACKs, timeouts, collation
...
## 8. Firmware update / bootloader protocol
...
command/status/ACK traffic (see §9). Example, for a 32-byte all-`0xAA`
chunk at offset 64:
...
- **No per-frame integrity check.** COBS only frames; MessagePack only
  ...
  checksum, or CRC on ordinary traffic (the XOR checksum in §8 covers only
...
| Concern | File(s) |
|---|---|
| Router RS485 transport, framing, send/receive, ACK/timeout | `Router/src/Modules/Hardware/RS485.cpp`, `.h` |
| Router per-Column/per-Portal routing, actions, keyframes | `Router/src/Modules/Hardware/Column.cpp`, `Portal.cpp` |
...
| Firmware RS485 framing, dispatch, DE-pin control | `PortalFW/src/Modules/RS485.cpp`, `.h` |
