<#
.SYNOPSIS
  test PortalTestBench. A wrapper; the implementation is test.mjs.
...
.DESCRIPTION
  ...
  Everything that was in here, including the reasons, moved to test.mjs unchanged.
  ...
  `powershell -File tools\test.ps1` is in people's shell history.
#>
[CmdletBinding()]
param([Parameter(ValueFromRemainingArguments = $true)] [string[]] $Rest)

$ErrorActionPreference = 'Stop'
node (Join-Path $PSScriptRoot 'test.mjs') @Rest
