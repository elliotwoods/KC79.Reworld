<#
.SYNOPSIS
  bootstrap PortalTestBench. A wrapper; the implementation is bootstrap.mjs.
...
.DESCRIPTION
  ...
  Everything that was in here, including the reasons, moved to bootstrap.mjs unchanged.
  ...
  `powershell -File tools\bootstrap.ps1` is in people's shell history.
#>
[CmdletBinding()]
param([Parameter(ValueFromRemainingArguments = $true)] [string[]] $Rest)

$ErrorActionPreference = 'Stop'
node (Join-Path $PSScriptRoot 'bootstrap.mjs') @Rest
